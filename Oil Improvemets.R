...
ggplot(event_results, aes(x = event_name, y = price_impact, fill = type)) +
  ...
  geom_text(
    aes(label = round(price_impact, 1)),
    vjust = ifelse(event_results$price_impact >= 0, -0.3, 1.3),
    size = 2.6
  ) +
  labs(
    title = "Oil Price Impact by Event",
    x = "Event",
    y = "% Price Change"
  ) +
  theme_minimal() +
...
event_results$type <- factor(event_results$type)

lm_model <- lm(price_impact ~ type, data = event_results)
summary(lm_model)
...
SELECT 
    type,
    ROUND(AVG(price_impact), 2) AS avg_impact,
    ROUND(MAX(price_impact), 2) AS max_impact,
    ROUND(MIN(price_impact), 2) AS min_impact,
    COUNT(*) AS num_events
FROM event_results
GROUP BY type
")
